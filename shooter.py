...
# test =pygame.image.load('dmh.jpg').convert_alpha()
...
def draw_bg(): # vẽ nền
    ...
    for x in range(5):
        ...
        # screen.blit(test, (x * width)- bg_scroll, SCREEN_HEIGHT  )
...
class Soldier(pygame.sprite.Sprite):
    def __init__(self,char_type, x, y, scale, speed, ammo, grenades):
        ...
        self.jump = False
        self.in_air = True  #kiểm tra nhảy để khi đang trên không, không thể nhảy tiếp
        self.flip = False
        self.animation_list = []
        self.frame_index = 0
        self.action = 0
    ...
    def move(self, moving_left, moving_right):
        ...
        if moving_left:
            dx -= self.speed
            self.flip = True        #di chuyển sang trái thì lật ảnh quay trái
            self.direction = -1
        if moving_right:
            dx += self.speed
            self.flip = False       #di chuyển sang phải thì lậi ảnh quay phải
            self.direction = 1
        ...
        for tile in world.obstacle_list:
            ...
            if tile[1].colliderect(self.rect.x , self.rect.y + dy, self.width, self.height):
                ...
                if self.vel_y < 0:
                    self.vel_y = 0
                    dy = tile[1].bottom - self.rect.top # khoảng cách để chạm đầu vào dưới vật cản
        ...
        #cập nhật cuộn màn hình dựa trên người chơi
        if self.char_type == 'player':
    ...
    def ai(self):
        if self.alive and player.alive:                 #địch tuần tra
            if random.randint(1,200) == 1 and self.idling == False:              #Dùng hàm random để tránh
                self.idling = True                                              #việc tất cả đều hành động giống nhau
                self.update_action(0)
                self.idling_counter = 50

            #kiem tra nguoi choi co trong tam nhin dich ko
        ...
        for y, row in enumerate(data):
            for x, tile in enumerate(row):
                if tile >= 0:
                    ...
                    elif tile >= 9 and tile <=10:#nước
                        water = Water(img,x * TILE_SIZE,y * TILE_SIZE)
                        water_group.add(water)
                    elif tile >= 11 and tile <=14:#ncỏ, đá
                        decoration = Decoration(img,x * TILE_SIZE,y * TILE_SIZE)
                        decoration_group.add(decoration)
                    elif tile ==  15 : #tạo player
                        player = Soldier('player',x * TILE_SIZE,y * TILE_SIZE,1.65,4,20,5)
                        health_bar = HealthBar(10, 10, player.health, player.max_health)
                    elif tile == 16 : #tạo địch
...
""" 3 lớp dưới có code giống nhau nhưng phải chia ra vì nó tương tác khác nhau với người chơi"""
...
class Bullet(pygame.sprite.Sprite):
    ...
    def update(self):
        ...
        #xoá đạn nếu ko nó vẫn tồn tại trong bộ nhớ
    ...
    def update(self):
        ...
        for tile in world.obstacle_list:
            if tile[1].colliderect(self.rect.x,self.rect.y, self.width, self.height):
                self.direction *= -1
                dx = self.speed * player.direction
            #kiểm tra va chạm theo trục y
            if tile[1].colliderect(self.rect.x , self.rect.y + dy, self.width, self.height):
                self.speed = 0
